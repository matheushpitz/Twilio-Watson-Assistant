...
namespace TwilioWatsonProject.Controllers
{
    ...
    public class TwilioWebhookController : TwilioController
    {
        ...
        public TwilioWebhookController(ITwilioService service, IWatsonService watsonService)
        {
            _service = service;
        ...
        public TwiMLResult Send()
        {
            ...
            //}    
            IEnumerable<string> watsonResult = this._watsonService.SendToAssistant(body, from);            
            foreach(string r in watsonResult)
            {
                response.Message(r);
            }
